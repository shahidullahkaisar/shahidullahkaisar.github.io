...
(function(){
  ...
  // one big painted anime cloud ☁️ (reused at different sizes)
  const CLOUD = `
    <svg viewBox="0 0 360 130" xmlns="http://www.w3.org/2000/svg">
      <ellipse cx="90"  cy="95" rx="80" ry="32" fill="#FFFFFF"/>
      <ellipse cx="180" cy="70" rx="95" ry="48" fill="#FFFFFF"/>
      <ellipse cx="265" cy="92" rx="78" ry="34" fill="#FFFFFF"/>
      <ellipse cx="140" cy="52" rx="55" ry="34" fill="#FFFFFF"/>
      <ellipse cx="225" cy="48" rx="45" ry="28" fill="#FDFDFD"/>
      <ellipse cx="180" cy="105" rx="150" ry="20" fill="#D9E9F2"/>
    </svg>`;

  // a floating island 🏝 — grassy top, rocky bottom, little tree, hanging roots
  const ISLAND = `
    <svg viewBox="0 0 220 210" xmlns="http://www.w3.org/2000/svg">
      <!-- hanging roots/vines -->
      <path d="M85 150 q -6 22 2 40" stroke="#6B8A5E" stroke-width="2.5" fill="none" stroke-linecap="round"/>
      <path d="M112 165 q 5 18 -2 34" stroke="#6B8A5E" stroke-width="2" fill="none" stroke-linecap="round"/>
      <path d="M132 145 q 8 20 3 36" stroke="#7A9468" stroke-width="2" fill="none" stroke-linecap="round"/>
      <!-- rock body -->
      <path d="M28 78 L45 118 L70 132 L88 160 L110 172 L128 152 L152 138 L172 108 L192 78 Z"
            fill="#8B7A64"/>
      <path d="M60 90 L78 128 L102 158 L110 172 L128 152 L118 120 L96 92 Z" fill="#6E6152" opacity=".8"/>
      <!-- grass top -->
      <ellipse cx="110" cy="74" rx="86" ry="26" fill="#5E9B60"/>
      <ellipse cx="102" cy="68" rx="66" ry="18" fill="#7CBB74"/>
      <!-- grass drips over the edge -->
      <path d="M30 80 q 6 12 14 6 M190 80 q -6 12 -14 6 M58 92 q 5 9 12 4" stroke="#5E9B60" stroke-width="5" fill="none" stroke-linecap="round"/>
      <!-- little tree -->
      <rect x="106" y="38" width="7" height="26" rx="3" fill="#7A5C40"/>
      <circle cx="110" cy="30" r="20" fill="#4E8A57"/>
      <circle cx="95"  cy="40" r="13" fill="#5E9B60"/>
      <circle cx="126" cy="40" r="13" fill="#5E9B60"/>
      <circle cx="110" cy="22" r="12" fill="#6FAF6B"/>
      <!-- tiny glowing crystal underneath ✨ -->
      <circle cx="110" cy="182" r="6" fill="#BFE9FF"/>
      <circle cx="110" cy="182" r="12" fill="#BFE9FF" opacity=".3"/>
    </svg>`;

  // a gliding bird 🕊
  const BIRD = `
    <svg viewBox="0 0 26 10" xmlns="http://www.w3.org/2000/svg">
      <path d="M1 8 Q 7 0 13 7 Q 19 0 25 8" stroke="#3C5A46" stroke-width="2" fill="none" stroke-linecap="round"/>
    </svg>`;
  ...
  sc.innerHTML = `
    ...
    <div class="island i1">${ISLAND}</div>
    <div class="island i2">${ISLAND}</div>
    <div class="island i3">${ISLAND}</div>
    <div class="bird b1">${BIRD}</div>
    <div class="bird b2">${BIRD}</div>
    <div class="bird b3">${BIRD}</div>
